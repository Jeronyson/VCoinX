...
| USERNAME | Логин для [автоматического получения токена](#автоматическое-получение-токена)             |
| PASSWORD | Пароль для [автоматического получения токена](#автоматическое-получение-токена)            |
| GROUP_ID | ID группы для майнинга коинов для группы                                                   |
...
Вы можете указать логин и пароль пользователя для автоматического получения токена и маскировки под официальное приложение одной из платформ (Android, iPhone, iPad, Windows Phone или Windows):
* в файле `config.js`, в переменных *USERNAME* и *PASSWORD*, соответственно.
* в аргументах запуска ключами *-u [username]* (или *-user [username]*) и *-p [password]* (или *-pass [password]*), соответственно.
По умолчанию, бот авторизуется как официальное приложение для Android. Если вы хотите изменить это поведение - используйте аргумент *-app [app]* и укажите одно из значений: android, iphone, ipad, windows_phone, windows.
> На данный момент бот не умеет обрабатывать капчу и двухфакторную авторизацию. Если вы получите соответствующее сообщение, либо отключите двухфакторную авторизацию, либо указывайте токен вручную.

> **Внимание:** в большинстве случаев при таком способе авторизации пользователь получит уведомление о входе с другого устройства!
...
* -u [username]		  - задать логин пользователя для автоматического получения токена.
* -p [password]		  - задать пароль пользователя для автоматического получения токена.
* -g [group_id]      - ID группы/паблика, для которой хотите майнить коины. (*прим.: -180610600*)
* -app [app]	  - указать вид приложения для автоматической авторизации (одно из значений: android, iphone, ipad, windows_phone, windows).
